...
const pool = new Pool({
  ...
  /* SSL requerido para bases de datos en la nube como Neon.
     En local (DB_SSL no definido) se conecta sin SSL. */
...
app.get('/api/barberias/:codigo', async (req, res) => {
  try {
    const r = await pool.query('SELECT id, codigo_unico, nombre, dueno_telefono, direccion, ciudad, horarios FROM barberias WHERE codigo_unico = $1 AND activa = true', [req.params.codigo]);
...
app.get('/api/mi-barberia', authMiddleware, async (req, res) => {
  try {
    const r = await pool.query('SELECT id,codigo_unico,nombre,dueno_nombre,dueno_email,dueno_telefono,direccion,ciudad,horarios,created_at FROM barberias WHERE id=$1', [req.user.id]);
    r.rows.length ? res.json(r.rows[0]) : res.status(404).json({ error: 'No encontrada' });
  } catch { res.status(500).json({ error: 'Error' }); }
});
...
app.put('/api/mi-barberia', authMiddleware, async (req, res) => {
  ...
  try {
    const r = await pool.query(`UPDATE barberias SET nombre=COALESCE($1,nombre), dueno_telefono=COALESCE($2,dueno_telefono),
      direccion=COALESCE($3,direccion), ciudad=COALESCE($4,ciudad), horarios=COALESCE($5,horarios), updated_at=CURRENT_TIMESTAMP
      WHERE id=$6 RETURNING id,codigo_unico,nombre,dueno_nombre,dueno_email,dueno_telefono,direccion,ciudad,horarios`,
      [nombre, dueno_telefono, direccion, ciudad, horarios, req.user.id]);
    res.json(r.rows[0]);
  } catch { res.status(500).json({ error: 'Error' }); }
});
