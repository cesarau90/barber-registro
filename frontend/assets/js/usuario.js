...
// Guard de autenticación: si no hay sesión válida, redirigir al login.
// Se ejecuta inmediatamente como IIFE para proteger toda la página.
(async () => {
...
/* ── TOAST ─────────────────────────────────────────────────────
   Implementación propia en vez de librería externa porque solo
   necesitamos 3 tipos (success/error/info) y mantener el bundle
   liviano. Se auto-destruye para evitar memory leaks por nodos
   acumulados en el DOM durante sesiones largas.
   ────────────────────────────────────────────────────────────── */
function toast(mensaje, tipo = 'info', duracion = 3000) {
    ...
    toastEl.innerHTML = `<i class="fas ${iconos[tipo]}"></i><span>${mensaje}</span>`;
    document.getElementById('toastContainer').appendChild(toastEl);
    setTimeout(() => {
        toastEl.classList.add('out');
        /* Espera 300ms para que la animación CSS de salida termine
           antes de remover el nodo del DOM */
...
/* ── HELPERS ───────────────────────────────────────────────────
   Funciones utilitarias compartidas por varias secciones.
   ────────────────────────────────────────────────────────────── */

/**
 * Formatea una fecha ISO (YYYY-MM-DD) a formato legible en español.
 * Usa substring para evitar problemas de timezone con new Date().
 */
function fmtFecha(f) {
    if (!f) return '-';
    try {
