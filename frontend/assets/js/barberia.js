...
/* ── TOAST ─────────────────────────────────────────────────────
   Duplicado de usuario.js en vez de módulo compartido porque
   barberia.js se carga en la página pública sin autenticación,
   mientras que usuario.js requiere sesión. Un módulo compartido
   agregaría una dependencia innecesaria a la página pública.
   ────────────────────────────────────────────────────────────── */
function toast(msg, type = 'info', duration = 3000) {
    const icons = { success: 'fa-check-circle', error: 'fa-exclamation-circle', info: 'fa-info-circle' };
    const el = document.createElement('div');
    el.className = `toast ${type}`;
    el.setAttribute('role', 'alert'); // Accesibilidad: lectores de pantalla anuncian el toast
    el.innerHTML = `<i class="fas ${icons[type]}"></i><span>${msg}</span>`;
    document.getElementById('toastContainer').appendChild(el);
    setTimeout(() => {
        el.classList.add('out');
        setTimeout(() => el.remove(), 300); // Espera a que termine la animación de salida
    }, duration);
}
...
async function cargarServicios() {
    ...
    cont.innerHTML = Array(3).fill(`
        <div style="width:260px;flex-shrink:0;">
            ...
            <div style="background:var(--surface);border:1px solid var(--border-2);border-top:none;border-radius:0 0 var(--radius-lg) var(--radius-lg);padding:15px 17px;">
    ...
    try {
        ...
        cont.innerHTML = servicios.map(s => {
            ...
            return `
            <div class="service-card" data-n="${s.nombre}" data-p="${s.precio}" tabindex="0" role="button" aria-label="Seleccionar servicio ${s.nombre} por $${s.precio}">
                <div class="service-img-wrapper">${imgHTML}</div>
...
async function cargarResenas() {
    ...
    try {
        ...
        cont.innerHTML = resenas.map((r, i) => {
            ...
            return `<div class="review-card fade-up" style="animation-delay:${i * .06}s">
